...
from models.user_info import UserCreate, UserSearchRequest, UserUpdate
...
async def search_user(user_search_request: UserSearchRequest) -> str:
    ...
    return await user_client.search_users(
        name=user_search_request.name,
        surname=user_search_request.surname,
        email=user_search_request.email,
        gender=user_search_request.gender,
    )
...
async def get_flow_diagram() -> bytes:
    """Get flow diagram"""
    with open(f"{os.path.dirname(__file__)}/flow.png", "rb") as f:
        return f.read()
...
@mcp.prompt()
async def ask_for_user_search() -> str:
...
@mcp.prompt()
async def ask_for_create_user() -> str:
